    ...
    >
      ...
      <div className="flex flex-col justify-center z-10 h-full w-[50%]   sm:w-[65%] md:w-[50%] gap-3 md:gap-5 ">
        <AnimatePresence mode="wait">
          <motion.div className="flex flex-wrap gap-2 md:mt-2 text-[0.6rem] font-dmsans_semibold sm:text-xs ">
            {fruit.highlights.map((item, i) => (
              <motion.span
                key={i}
              ...
              >
                ...
                {/* Responsive Text */}
                <motion.span
                  key={fruitData[index].color} // key makes motion detect change
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  transition={{ duration: 1.2 }}
                  className="whitespace-nowrap text-[0.6rem] md:text-xs"
                >
                  {item}
                </motion.span>
              </motion.span>
