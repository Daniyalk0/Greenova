      ...
      <div className="flex items-center justify-center w-full flex-col sm:flex-row gap-3 md:gap-6 lg:gap-14">
        <div className="flex items-center justify-center flex-col w-full md:w-[55%] lg:w-[50%] ">
          <div className="flex flex-col items-start justify-start gap-2 sm:gap-4">
            <h2 className="font-monasans_semibold text-[0.4rem] sm:text-[0.7rem] tracking-wider uppercase lg:text-[0.6rem]">
              Your Words Matters ⸺
            </h2>
            <h1 className="font-playfair text-2xl sm:text-5xl">
              What People Say About Us
            </h1>
            <p className="font-dmsans_light text-[0.9rem] sm:text-[1.1rem] md:text-[1rem] leading-tight sm:leading-4 md:leading-5">
              From first impressions to lasting experiences, our customers share
              the moments that made their journey smooth, memorable, and worth
              coming back for every single time.
            </p>
          </div>
          {reviews.length > 0 ? (
            <Marquee
          ...
          {reviews.length > 0 ? (
            <Marquee
              ...
              gradient
            >
              <div className=" gap-8 pr-8 flex md:hidden">
                {" "}
                {/* 👈 add this padding */}
                {reviews.map((review, i) => (
                  <ReviewCard
        ...
        <AddReview
          isUserHasReview={isUserHasReview}
          setIsUserHasReview={setIsUserHasReview}
        />
      </div>
    </section>
  );
